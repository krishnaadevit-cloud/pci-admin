...
export default function DocumentUploadStep({
  ...
  // Sync initialDocuments in case they arrive after the component mounts
  // (mirrors how fresh-registration DocumentUploadStep handles Redux prefill)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!addQualApplicationType) return;

    const fetchDocuments = async () => {
      setLoadingDocs(true);
      try {
        const response = await getAllDetails(
          `${MDM_APPLICATION_DOCUMENTS}?applicationName=${encodeURIComponent(addQualApplicationType.applicationName)}`,
        );
  ...
  const handleMakePayment = async () => {
    ...
    try {
      ...
      const response = await paymentPostData(CREATE_PAYMENT_ORDER, {
        ...
        applicationName: addQualApplicationType?.applicationName,
      });
