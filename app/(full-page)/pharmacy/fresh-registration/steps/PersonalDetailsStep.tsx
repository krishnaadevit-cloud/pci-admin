...
import PersonalDetailsMainSection from "../forms/PersonalDetailsMainSection";
import PersonalContactSection from "../forms/PersonalContactSection";
import PersonalDetailsFooter from "../forms/PersonalDetailsFooter";
...
export default function PersonalDetailsStep({ onBack, onContinue, validationTrigger, onScrollToTop }: PersonalDetailsStepProps) {
  ...
  useEffect(() => {
    ...
    const derivedFullName = [form.surname, form.firstName, form.middleName].filter(Boolean).join(" ");
    if (!form.fullName && derivedFullName) {
