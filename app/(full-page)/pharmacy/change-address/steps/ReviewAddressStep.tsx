...
export default function ReviewAddressStep({ onBack, onContinue }: ReviewAddressStepProps) {
  ...
  useEffect(() => {
    if (!dashboardData) {
      dispatch(fetchDashboardData()).unwrap().catch((err: unknown) => {
        const message = (err as { message?: string })?.message ?? "Failed to load address details. Please try again.";
  ...
  const permanentAddress = cd.permanent_address ?? "—";
  const permanentPinCode = cd.permanent_pin_code ?? "—";
  const permanentState = cd.permanent_state ?? "—";
  const permanentDistrict = cd.permanent_district ?? "—";
  const permanentCity = cd.permanent_city ?? "—";
      ...
      <div className="prts-personal-form-card">
        <div style={{ borderBottom: "1px solid #E5E5E5" }} className="prts-step-content pb-6">
          <div className="prts-step-content__header">
            <h1 className="prts-step-content__title m-0">Review Address Details</h1>
          </div>
        </div>
        ...
        <div className="prts-application-preview prts-application-preview--inline mt-6">
          ...
          <div className="prts-preview-grid prts-preview-grid--4">
            <PreviewField label="Pin Code" value={permanentPinCode} />
            <PreviewField label="State" value={permanentState} />
            <PreviewField label="District" value={permanentDistrict} />
            <PreviewField label="City" value={permanentCity} />
          </div>
        </div>
