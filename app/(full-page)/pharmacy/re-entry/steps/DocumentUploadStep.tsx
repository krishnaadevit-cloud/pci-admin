...
export default function DocumentUploadStep({
  ...
  const reEntryApplicationType = useAppSelector(selectApplicationTypeByName("Re Entry Registration"));
  ...
  useEffect(() => {
    ...
    if (!reEntryApplicationType) return;

    const fetchDocuments = async () => {
      setLoadingDocs(true);
      try {
        const response = await getAllDetails(
          `${MDM_APPLICATION_DOCUMENTS}?applicationName=${encodeURIComponent(reEntryApplicationType.applicationName)}`,
        );
  ...
  const handleUploadComplete = async (id: number, file: File) => {
    ...
    formData.append("application_uuid", reEntryApplicationType?.uuid ?? "");
  ...
  const handleMakePayment = async () => {
    ...
    try {
      ...
      const response = await paymentPostData(CREATE_PAYMENT_ORDER, {
        ...
        applicationName: reEntryApplicationType?.applicationName,
      });
