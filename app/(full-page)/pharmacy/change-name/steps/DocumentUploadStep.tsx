...
export default function DocumentUploadStep({
  ...
  useEffect(() => {
    ...
    if (!changeNameApplicationType) return;

    const fetchDocuments = async () => {
      setLoadingDocs(true);
      try {
        const response = await getAllDetails(
          `${MDM_APPLICATION_DOCUMENTS}?applicationName=${encodeURIComponent(changeNameApplicationType.applicationName)}`,
        );
        if (Array.isArray(response) && response.length > 0) {
          const savedDocs: any[] = dashboardData?.name_change_details?.other_documents ?? [];
          const rows: DocumentRowState[] = response.map((item: any, index: number) => {
            const saved = savedDocs.find(
              (d) => d.document_uuid === item.documentUuid || d.document_name === item.documentName,
            );
            if (saved?.download_url) {
              return {
                id: item.id ?? index + 1,
                uuid: saved.uuid,
                ...
                selected: true,
                fileName: saved.download_url.split("/").pop() ?? "document.pdf",
                fileSize: "—",
  ...
  const handleUploadComplete = async (id: number, file: File) => {
    ...
    formData.append("application_uuid", changeNameApplicationType?.uuid ?? "");
