...
export default function DocumentUploadStep({
  ...
  useEffect(() => {
    ...
    if (!renewalApplicationType) return;

    const fetchDocuments = async () => {
      setLoadingDocs(true);
      try {
        const response = await getAllDetails(
          `${MDM_APPLICATION_DOCUMENTS}?applicationName=${encodeURIComponent(renewalApplicationType.applicationName)}`,
        );
        if (Array.isArray(response) && response.length > 0) {
          const savedDocs: any[] = dashboardData?.renewal_details?.other_documents ?? [];
