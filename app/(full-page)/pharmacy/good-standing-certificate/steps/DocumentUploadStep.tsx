...
export default function DocumentUploadStep({
  ...
  useEffect(() => {
    ...
    if (!goodStandingApplicationType) return;

    const fetchDocuments = async () => {
      setLoadingDocs(true);
      try {
        const response = await getAllDetails(
          `${MDM_APPLICATION_DOCUMENTS}?applicationName=${encodeURIComponent(goodStandingApplicationType.applicationName)}`,
        );
  ...
  const handleUploadComplete = async (id: number, file: File) => {
    ...
    formData.append("application_uuid", goodStandingApplicationType?.uuid ?? "");
