...
const StateCouncilDashboard = () => {
  ...
  useEffect(() => {
    // Sync dropdown state with route params
  ...
  const handleStateChange = (e: any) => {
    ...
    if (val === 'all') {
      ...
      router.push(`/super-admin/state-council/${val}`);
    }
  };
  ...
  return (
    <div className="super-admin-dashboard">
      ...
      <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center gap-3 dashboard-header">
        <div>
          <h2 className="header-title">{stateData.stateName}</h2>
          <p className="header-subtitle">State Council Oversight Dashboard & Analytics</p>
        </div>
      ...
      <div className="grid gap-3 mb-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
        {stateData.summaryCards.map((card) => (
          <div 
            ...
            onClick={() => {
              if (card.id === 'applications' || card.id === 'pending' || card.id === 'approved' || card.id === 'rejected') {
                router.push(`/super-admin/applications/state-wise/${stateId}`);
              }
            }}
          >
          ...
          <div className="dashboard-panel">
            <div className="panel-header flex justify-content-between align-items-center">
              ...
              <Button 
                ...
                onClick={() => router.push(`/super-admin/applications/state-wise/${stateId}`)}
              />
            </div>
