...
const GrievanceSupport = () => {
  ...
  const normalizedState = useMemo(() => {
    if (stateId === 'all') return 'all';
    return stateId.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
  }, [stateId]);

  const filteredTickets = useMemo(() => {
    if (normalizedState === 'all') return tickets;
    return tickets.filter(t => t.state === normalizedState);
  }, [tickets, normalizedState]);
    ...
    <div className="super-admin-dashboard">
        ...
        <div className="fal-page-header__inner">
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            ...
            <div className="fal-page-header__text">
              <h1 className="fal-page-header__title">IT Grievances & Support Desk</h1>
              <p className="fal-page-header__subtitle">Resolve helpdesk tickets raised by pharmacists, colleges, and council admins</p>
            </div>
          </div>
      ...
      <div className="dashboard-panel">
        ...
        <DataTable 
          ...
          globalFilterFields={['id', 'applicantName', 'category', 'state']} 
          paginator 
          rows={10} 
          className="p-datatable-striped" 
          emptyMessage={<EmptyTableMessage message="No IT grievances found matching the selection." />}
        ...
        >
          ...
          <Column 
            ...
            body={(row) => {
              ...
              const badgeClass = 
                priorityLower === 'critical' || priorityLower === 'high' ? 'fal-status--rejected' :
                priorityLower === 'medium' ? 'fal-status--pending' :
                'fal-status--approved';
              return (
                <span className={`fal-status ${badgeClass}`}>
                  {row.priority}
                </span>
              );
            }}
          ...
          <Column 
            ...
            body={(row) => {
              ...
              const badgeClass = 
                statusLower === 'resolved' || statusLower === 'closed' ? 'fal-status--approved' :
                statusLower === 'in progress' ? 'fal-status--in_review' :
                'fal-status--pending';
              return (
                <span className={`fal-status ${badgeClass}`}>
                  {row.status}
