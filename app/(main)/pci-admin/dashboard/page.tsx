...
const NationalDashboard = () => {
  ...
  useEffect(() => {
    ...
    setAppStatsOptions({
      ...
      onClick: (event: any, elements: any, chart: any) => {
        if (elements && elements.length > 0) {
          ...
          router.push(
            `/pci-admin/applications/state-wise/all?type=${encodeURIComponent(
              label,
            )}`,
          );
        }
    ...
    setStatusOverviewData({
      labels: nationalStats.applicationStatusOverview.map(
        (item) => item.status,
      ),
      datasets: [
        {
          data: nationalStats.applicationStatusOverview.map(
            (item) => item.value,
          ),
          backgroundColor: nationalStats.applicationStatusOverview.map(
            (item) => item.color,
          ),
          hoverBackgroundColor: nationalStats.applicationStatusOverview.map(
            (item) => item.color + "dd",
          ),
        },
      ],
    ...
    setStatusOverviewOptions({
      ...
      onClick: (event: any, elements: any, chart: any) => {
        if (elements && elements.length > 0) {
          ...
          const label = chart.data.labels[index];
          router.push(
            `/pci-admin/applications/state-wise/all?status=${encodeURIComponent(
              label,
            )}`,
          );
        }
  ...
  const getAlertIcon = (severity: string) => {
    switch (severity) {
      case "high":
        return "pi pi-exclamation-triangle alert-icon";
      case "medium":
        return "pi pi-info-circle alert-icon";
      default:
        return "pi pi-bell alert-icon";
    }
  };

  // Mock colors for pharmacist map legend
  const getMapColor = (stateId: string) => {
    const match = nationalStats.statewisePharmacistsDistribution.find(
      (s) => s.stateId === stateId,
    );
        ...
        <div className="fal-page-header__inner">
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            ...
            <div className="fal-page-header__text">
              <h1 className="fal-page-header__title">National Dashboard</h1>
              <p className="fal-page-header__subtitle">
                Pharmacy Council of India (PCI) • Super Admin Control Center
              </p>
            </div>
          </div>
          ...
          <div
            ...
            onClick={() => {
              if (card.id === "states") {
                ...
                router.push(
                  "/pci-admin/applications/state-wise/all?status=Pending",
                );
              } else if (card.id === "approved") {
                router.push(
                  "/pci-admin/applications/state-wise/all?status=Approved",
                );
              } else if (card.id === "rejected") {
                router.push(
                  "/pci-admin/applications/state-wise/all?status=Rejected",
                );
              } else {
                router.push("/pci-admin/applications/state-wise/all");
              }
        ...
        <div className="grid-col-4">
          <div className="dashboard-panel">
            ...
            <div className="text-center mt-3">
              <span className="text-2xl font-bold text-900">2,54,120</span>
              <span className="text-xs text-500 block">
                Total Applications Under Scope
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="panel-header flex-column md:flex-row md:justify-content-between md:align-items-center gap-2">
            <div>
              ...
              <p className="text-xs text-500 m-0 mt-1">
                Click on any row to open the specific State Council dashboard
              </p>
            </div>
