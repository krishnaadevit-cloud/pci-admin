  ...
  return (
    <div className="super-admin-dashboard">
      ...
      <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center gap-3 dashboard-header">
        <div>
          <h2 className="header-title">Institutions & Colleges Registry</h2>
          <p className="header-subtitle">Manage approved pharmacy colleges, track intake capacity, and schedule PCI inspections</p>
        </div>
      </div>
      ...
      <div className="dashboard-panel">
        ...
        <DataTable
          ...
          paginator rows={10} className="p-datatable-striped" responsiveLayout="scroll"
        >
          ...
          <Column 
            ...
            body={(row) => <Tag severity={getStatusSeverity(row.approvalStatus)} value={row.approvalStatus} />}
          />
          <Column 
            header="Actions" 
            body={(row) => (
              <div className="flex gap-1">
                <Button icon="pi pi-pencil" className="p-button-rounded p-button-text p-button-sm" onClick={() => editInst(row)} tooltip="Edit Details" tooltipOptions={{ position: 'bottom' }} />
                <Button icon="pi pi-calendar-plus" className="p-button-rounded p-button-text p-button-info p-button-sm" onClick={() => toast.current?.show({severity:'success', summary:'Inspection Scheduled', detail:`Notification sent to ${row.name}`})} tooltip="Schedule Inspection" tooltipOptions={{ position: 'bottom' }} />
                <Button icon="pi pi-trash" className="p-button-rounded p-button-text p-button-danger p-button-sm" onClick={() => { setInstitutions(institutions.filter(i => i.id !== row.id)); toast.current?.show({severity:'warn', summary:'Deleted', detail:'Institution removed'}) }} tooltip="Remove" tooltipOptions={{ position: 'bottom' }} />
              </div>
            )}
          />
        </DataTable>
