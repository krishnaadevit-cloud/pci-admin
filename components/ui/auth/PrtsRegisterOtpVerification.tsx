...
export default function PrtsOtpVerification({ flow }: PrtsOtpVerificationProps) {
  ...
  // Start once on mount using the cookie-derived remaining time; page refresh
  // will read the cookie again and continue from the remaining seconds.
  useEffect(() => {
    const expiresAt = getOtpExpiresAt();
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      clearOtpFlowCookies();
      completeLogin({
        fullName: response?.data?.user?.fullName ?? response?.data?.fullName ?? "User",
        email: pending?.email,
      ...
      <article className="prts-auth-card">
        ...
        <form className="prts-auth-form2" onSubmit={handleVerify} noValidate>
          ...
          <div className="prts-auth-otp-meta">
            {(flow === "login" || flow === "register") && secondsLeft > 0 ? (
